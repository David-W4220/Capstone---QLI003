...
const InventoryApp = () => {
    ...
    try {
        const response = await fetch(REPORT_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
        const data = await response.json();
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-8 font-sans">
            ...
            <button
                ...
                className="py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-gray-400 ml-4"
                disabled={reportStatus.includes('Generating')}
                >                    
                {reportStatus.includes('Generating') ? 'Generating...' : 'Generate History Report'}
            </button>
            {/*inline status display for mail*/}
            {reportStatus && (
                <p className={`mt-3 text-center font-bold ${reportStatus.includes('failed')
                 //red if error and blue otherwise
