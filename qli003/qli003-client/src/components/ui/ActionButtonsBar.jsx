...
// Component now accepts userRole prop
const ActionButtonsBar = ({ 
  ...
  onQuickCheckOut,
  isGeneratingReport,
  userRole // <-- NEW: Accept userRole prop
}) => {
  ...
  return (
    <div className="mb-6 flex flex-wrap gap-3">
      ...
      {/* Quick Check Out Button (This is a Staff/Public Action, so always visible if handler is provided) */}
      {onQuickCheckOut && (
        <button 
          onClick={onQuickCheckOut}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 transition-colors btn-scale"
        >
          <svg 
            className="h-4 w-4" 
            fill="none" 
      ...
      {/* Generate History Report Button (ADMIN ONLY) */}
      {isAdmin && (
        <button 
